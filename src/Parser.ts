...
export class Lexer {
    ...
    // Takes in Syntax Error code and string objects, returns a detailed error message
    error(code: SyntaxErrorCode, msg: string) {
        const pos = this.next_position();
        return new SyntaxError(code, { start: pos, end: pos }, msg);
    }
    // Takes in TokenType and value, returns a token object
    ...
    // Returns the next character of the lexer
    next_char() {
...
// Parser takes in tokens generated by the lexer and converts them into the actual program
export class Parser {
    tokens: Token[]
